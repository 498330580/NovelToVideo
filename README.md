...
- **图像处理**: Pillow 10.1.0
- **视频处理**: moviepy 1.0.3
- **视频拼接**: FFmpeg (使用 -c copy 参数快速无损拼接)
- **数据库**: SQLite 3
- **运行环境**: Python 3.10
...
系统采用**队列驱动**的视频合成架构，确保按顺序处理每个队列：

**5步处理流程**（每个队列）：
1. **生成临时视频** - 为该队列的所有临时视频片段生成视频，支持断点续传
2. **合并视频** - 使用 FFmpeg `-c copy` 参数直接拼接所有临时视频为最终输出（无需重新编码，速度快）
3. **标记完成** - 最终视频合成完毕后立即将队列状态改为 COMPLETED
4. **删除临时文件** - 删除对应的临时视频文件，并标记数据库为 DELETED
5. **创建记录** - 为最终视频创建数据库记录

**关键特性**：
- ✅ **严格顺序处理** - 前一个队列失败时停止整个流程，防止跳队
- ✅ **断点续传** - 支持异常中断后继续处理（检查临时视频状态）
- ✅ **内存优化** - 临时视频及时删除，不占用长期内存
- ✅ **快速拼接** - FFmpeg `-c copy` 参数避免重新编码
- ✅ **实时进度** - 动态更新任务进度百分比
...
**Q: 视频拼接时出现编码错误?**
A: 这通常是因为项目名称或路径包含中文，已修复此问题。确保使用最新版本的代码。

**Q: FFmpeg 无法找到临时视频文件?**
A: 检查 concat_list.txt 文件的换行符格式是否正确。确保系统上的 FFmpeg 版本不过旧。
...
**视频拼接编码问题**
- 原问题：FFmpeg 拼接时无法处理中文路径，出现 `UnicodeDecodeError: 'gbk' codec can't decode`
- 原因：FFmpeg subprocess 默认使用系统编码（Windows 为 GBK）
- 解决方案：指定 `encoding='utf-8'` 和 `errors='replace'` 处理中文路径
- 效果：正确支持包含中文的项目名称和路径

**视频拼接文件格式问题**
- 原问题：FFmpeg concat_list.txt 文件格式错误，导致无法识别视频文件
- 原因：换行符处理不当，使用了转义字符而非实际换行
- 解决方案：修改换行符处理，使用正确的 `\n` 换行符
- 效果：FFmpeg 能正确解析 concat 文件列表

**队列处理顺序问题**
- 原问题：某个队列处理失败时，系统仍继续处理下一个队列
- 原因：异常处理中使用 `continue` 而非 `return False`
- 解决方案：队列处理失败时立即停止整个合成流程
- 效果：严格按照用户要求的顺序处理队列，失败时停止而不跳队
...
6. **文件清理**: 删除项目时会自动清理相关的临时文件，包括：
   ...
   - 临时视频片段目录
   - 项目使用的自定义背景图片
7. **队列处理顺序**: 系统采用队列驱动架构，严格按照队列顺序处理视频。一个队列处理失败会导致整个流程停止，需要重新开始。
8. **中文路径支持**: 系统已完全支持包含中文的项目名称、路径和文件名。
9. **FFmpeg 依赖**: 确保系统已安装 FFmpeg，系统使用 FFmpeg 的 `-c copy` 参数进行快速无损视频拼接。
